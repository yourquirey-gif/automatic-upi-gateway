...
function withinOrderWindow(date, order) {
  ...
  const start = new Date(order.createdAt).getTime() - 10 * 60 * 1000;
  const end = Math.min(Date.now() + 5 * 60 * 1000, new Date(order.createdAt).getTime() + 48 * 60 * 60 * 1000);
  return t >= start && t <= end;
}

async function listPaymentMessages(gmail, settings) {
  const q = settings?.gmailSearchQuery || 'newer_than:2d';
...
async function saveReceipt({ merchant, message, utr, amount, merchantUpiId, payerUpiId, receivedAt }) {
  ...
  try {
    return await PaymentReceipt.create({
      merchant: merchant._id,
      messageId: message.id,
      threadId: message.threadId || null,
      utr,
      amount,
      merchantUpiId: normalizeUpi(merchantUpiId),
...
async function verifyConnection(connection, settings) {
  ...
  if (!merchant || merchant.status !== 'active' || merchant.verificationStatus !== 'verified') return { checked: 0, confirmed: 0, subscriptionsActivated: 0, kycPaymentsConfirmed: 0 };
  ...
  for (const id of ids) {
    ...
    // If two pending orders have the same amount, do not guess. An ambiguous payment stays PENDING.
  ...
  return { checked: ids.length, confirmed, subscriptionsActivated: 0, kycPaymentsConfirmed: 0 };
}
...
export async function verifyMerchantGmail({ merchant, client, email, refreshToken }) {
  ...
  for (const id of ids) {
    ...
    const mobileMatches = merchant.mobile && String(text).replace(/\D/g, '').includes(String(merchant.mobile).replace(/\D/g, ''));
  ...
  if (!matched) {
    ...
    merchant.verificationMessage = 'Google login succeeded, but no payment email proving this Gmail account is linked to the merchant UPI account was found. Please use the Gmail account linked with this merchant/payment account.';
    await merchant.save();
    return { verified: false, message: merchant.verificationMessage };
  }
  await GmailConnection.findOneAndUpdate(
    { merchant: merchant._id },
    { merchant: merchant._id, owner: merchant.owner, email, refreshTokenEncrypted: encryptSecret(refreshToken), active: true },
    { upsert: true, new: true }
  );
  merchant.verificationStatus = 'verified';
  merchant.status = 'active';
  merchant.verifiedAt = new Date();
  merchant.verifiedEmail = email;
...
export async function verifyPendingOrdersForAdmin(ownerId) {
  ...
  if (!connections.length) return { checked: 0, confirmed: 0, subscriptionsActivated: 0, kycPaymentsConfirmed: 0, reason: 'gmail_not_connected' };
  ...
  const totals = { checked: 0, confirmed: 0, subscriptionsActivated: 0, kycPaymentsConfirmed: 0 };
