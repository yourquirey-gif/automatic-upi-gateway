...
function oauthClient() { return new google.auth.OAuth2(process.env.GOOGLE_CLIENT_ID, process.env.GOOGLE_CLIENT_SECRET, process.env.GOOGLE_REDIRECT_URI); }
...
function extractUtr(text) { return text.match(/(?:UTR|UPI\s*Ref(?:erence)?|Transaction\s*ID)[^A-Za-z0-9]{0,20}([A-Za-z0-9-]{8,40})/i)?.[1] || null; }
...
export async function verifyPendingOrdersForAdmin(ownerId) {
  ...
  const client = oauthClient(); client.setCredentials({ refresh_token: decryptSecret(connection.refreshTokenEncrypted) });
  ...
  for (const id of ids) {
    ...
    for (const order of pending) {
      ...
      order.status = 'SUCCESS'; order.paidAt = new Date(); order.utr = utr || order.utr; order.feeAmount = fee; order.netAmount = Number((order.amount - fee).toFixed(2)); order.feeSettlementStatus = fee > 0 ? 'PENDING' : 'NOT_APPLICABLE'; order.verificationSource = 'gmail'; order.verificationMessageId = id;
    ...
    for (const sub of subscriptions) {
      ...
      const started = new Date(); const expires = expiryFrom(started, sub.plan.durationDays);
      sub.status = 'SUCCESS'; sub.paidAt = started; sub.utr = utr || sub.utr; sub.planActivatedAt = started; sub.planExpiresAt = expires; sub.verificationSource = 'gmail'; sub.verificationMessageId = id; await sub.save();
      await User.findByIdAndUpdate(sub.user, { plan: sub.plan._id, planStartedAt: started, planExpiresAt: expires, planStatus: 'ACTIVE', trialStartedAt: started, trialEndsAt: expires }); subscriptionsActivated += 1;
    }
    for (const kyc of kycPayments) {
      if (kyc.status !== 'PENDING_PAYMENT' || !text.toLowerCase().includes(kyc.orderId.toLowerCase()) || !amountMatches(text, kyc.amount)) continue;
      const paidAt = new Date(); kyc.status = 'SUBMITTED'; kyc.paidAt = paidAt; kyc.submittedAt = paidAt; kyc.utr = utr || kyc.utr; kyc.verificationSource = 'gmail'; kyc.verificationMessageId = id; await kyc.save();
      await User.findByIdAndUpdate(kyc.user, { kycStatus: 'PENDING_REVIEW' }); kycPaymentsConfirmed += 1;
    }
  }
