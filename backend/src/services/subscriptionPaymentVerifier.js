...
function extractAmounts(text) { const normalized = String(text).replace(/,/g, ''); return [...new Set([...normalized.matchAll(/(?:₹|INR|Rs\.?)[\s:]*([0-9]+(?:\.[0-9]{1,2})?)/gi)].map(m => Number(m[1])).filter(Number.isFinite))]; }
...
async function activate(order, receivedAt, utr, messageId, merchant) {
  if (order.status !== 'PENDING' || !order.plan) return false;
  const started = new Date(receivedAt);
  const expires = expiryFrom(started, order.plan.durationDays);
  order.status = 'SUCCESS'; order.paidAt = started; order.utr = utr || order.utr; order.planActivatedAt = started; order.planExpiresAt = expires; order.verificationSource = 'gmail'; order.verificationMessageId = messageId;
  await order.save();
  await User.findByIdAndUpdate(order.user, { plan: order.plan._id, planStartedAt: started, planExpiresAt: expires, planStatus: 'ACTIVE' });
  if (merchant && merchant.verificationStatus !== 'verified') { merchant.verificationStatus = 'verified'; merchant.status = 'active'; merchant.verifiedAt = new Date(); merchant.verificationMessage = 'Settlement UPI verified from a matching payment notification.'; await merchant.save(); }
  return true;
}
...
export async function verifySubscriptionOrderForAdmin(orderId) {
  ...
  for (const admin of adminUsers) {
    const merchant = await Merchant.findOne({ owner: admin._id, provider: 'admin_settlement', upiId: { $exists: true, $nin: ['', null] } }).sort({ verifiedAt: -1, createdAt: -1 });
    if (!merchant) continue;
    const targetUpi = normalizeUpi(merchant.upiId);
    if (!targetUpi) continue;
    const connection = await GmailConnection.findOne({ owner: admin._id, merchant: merchant._id, active: true }).select('+refreshTokenEncrypted')
      || await GmailConnection.findOne({ owner: admin._id, active: true }).select('+refreshTokenEncrypted');
    ...
    try {
      ...
      const listed = await gmail.users.messages.list({ userId: 'me', q: 'newer_than:2d', maxResults: 100 });
      for (const item of listed.data.messages || []) {
        ...
        if (!/(credited|received|payment|upi|transaction|collect)/i.test(text)) continue;
        const utr = extractUtr(text);
        const ok = await activate(order, receivedAt, utr, item.id, merchant);
        if (ok) { connection.lastCheckedAt = new Date(); connection.lastMessageId = item.id; await connection.save(); return { verified: true, status: 'SUCCESS', paidAt: order.paidAt, expiresAt: order.planExpiresAt, utr: order.utr }; }
      }
      connection.lastCheckedAt = new Date(); await connection.save();
    } catch (error) { console.error(`Subscription Gmail verification failed for ${connection.email}:`, error.message); }
  }
