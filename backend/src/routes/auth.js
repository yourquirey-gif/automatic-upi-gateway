...
// Merchant onboarding intentionally supports both normal users and administrators.
// An administrator is still kept as an administrator; this flow only creates/updates
// the payment merchant record owned by that account. It must not redirect to the
// administrator-login error page when the admin uses the normal user panel.
...
router.get('/google/callback',async(req,res,next)=>{try{
  ...
  // IMPORTANT: do NOT reject administrators here. The merchant onboarding flow is
  // also used by an administrator who opens the user panel. The administrator role
  // remains unchanged; only the merchant verification record is updated.
  let user=await User.findOne({$or:[{email},{googleId}]}).select('+passwordHash');
  if(!user){
    const {started,ends}=trialDates(),userId=await nextUserId(),{apiToken,instanceSecret}=createApiCredentials(),passwordHash=await bcrypt.hash(crypto.randomBytes(32).toString('hex'),12);
    user=await User.create({name,email,passwordHash,authProvider:'google',googleId,userId,apiToken,instanceSecret,webhookUrl:'',trialStartedAt:started,trialEndsAt:ends});
