...
const r=Router();
r.get('/',requireAuth,requireAdmin,async(_q,res,next)=>{try{const c=await KycConfig.findOneAndUpdate({key:'global'},{},{upsert:true,new:true,setDefaultsOnInsert:true});res.json({status:true,config:c});}catch(e){next(e)}});
