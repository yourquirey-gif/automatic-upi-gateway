...
router.get('/:orderId', async (req, res, next) => {
  try {
    ...
    res.json({ status: true, result: { orderId: order.orderId, amount: Number(order.amount).toFixed(2), status: order.status, expiresAt: order.expiresAt, redirectUrl: order.redirectUrl || null, merchant: { name: merchant.name, provider: merchant.provider }, verification: verificationCheckout, checkout: { brandName: clean(c.brandName || merchant.name, 100), themeColor: /^#[0-9a-fA-F]{6}$/.test(c.themeColor || '') ? c.themeColor : '#0B95BD', instructions: clean(c.instructions || '', 3000), showQrCode: c.showQrCode !== false, showIntentButtons: c.showIntentButtons !== false, showUpiId: false, showCopyButton: false, showBhim: c.showBhim !== false, brandLogo: typeof c.brandLogo === 'string' ? c.brandLogo : '' }, upiUrl: paymentUpiUrl, qrDataUrl } });
