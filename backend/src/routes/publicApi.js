...
function buildPaymentUrl(req, order) { const base = String(process.env.PUBLIC_API_BASE_URL || `${req.protocol}://${req.get('host')}`).replace(/\/$/, ''); return `${base}/api/payment/${encodeURIComponent(order.orderId)}`; }
...
// Public, read-only status endpoint used only by the hosted checkout page.
// It intentionally exposes no API token, webhook secret or merchant credentials.
...
router.get('/health', (_req, res) => res.json({ status: true, service: 'Automatic UPI Gateway Public API', version: '1.2' }));
