...
function isExpired(order) { return order.status === 'PENDING' && new Date(order.expiresAt).getTime() <= Date.now(); }
async function expireOrderIfNeeded(order) { if (isExpired(order)) { await Order.updateOne({ _id: order._id, status: 'PENDING' }, { $set: { status: 'EXPIRED' } }); return true; } return false; }
...
router.get('/payment/:orderId', async (req, res, next) => {
  try {
    ...
    const qrBlock = c.showQrCode ? `<div class="scan"><span class="scanTag">▦ &nbsp; Scan & Pay</span><div class="qrLine"><img id="qr" src="${qr}" alt="UPI QR"><div><a download="payment-qr.png" href="${qr}" class="download" style="background:${c.themeColor}">⇩ &nbsp; Download QR</a></div></div></div>` : '';
