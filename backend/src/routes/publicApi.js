...
router.post('/create-order', requireApiUser, async (req, res, next) => {
  try {
    ...
    const order = await Order.create({ merchant: merchant._id, owner: req.apiUser._id, orderId, amount: amountFixed, customerMobile, redirectUrl, remark1, remark2, status: 'PENDING', feePercent, netAmount: amountFixed, paymentUrl: '', expiresAt });
    order.paymentUrl = buildPaymentUrl(req, order); await order.save();
    const upiUrl = buildUpiUrl(order, merchant);
    res.status(201).json({ status: true, message: 'Order Created Successfully', result: { txnStatus: 'PENDING', orderId: order.orderId, order_id: order.orderId, amount: order.amount.toFixed(2), paymentUrl: order.paymentUrl, payment_url: order.paymentUrl, upiUrl, upi_url: upiUrl, expiresAt: order.expiresAt, expires_at: order.expiresAt, expiresInSeconds: 300, redirectUrl: order.redirectUrl || null, customerMobile: order.customerMobile || null, remark1: order.remark1 || null, remark2: order.remark2 || null } });
