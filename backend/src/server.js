...
async function gmailAutoSyncTick() { const settings = await GatewaySettings.findOne({ key: 'global' }).lean(); const enabled = settings?.gmailAutoSync !== false && process.env.GMAIL_AUTO_SYNC !== 'false'; if (!enabled) return; try { const result = await verifyAllConnectedGmails(); if (result.confirmed) console.log('Gmail IMAP verification:', result); } catch (e) { console.error('Gmail IMAP sync failed:', e.message); } }
...
connectDatabase().then(async () => { await repairUniqueIndexes(); await ensureBootstrapAdmin(); app.listen(port, () => console.log(`OmniUPI API listening on port ${port}`)); await expireSubscriptions().catch(e => console.error('Initial expiry check failed:', e.message)); setInterval(() => expireSubscriptions().catch(e => console.error('Subscription expiry check failed:', e.message)), Number(process.env.SUBSCRIPTION_EXPIRY_CHECK_MS || 60000)); setInterval(gmailAutoSyncTick, Number(process.env.GMAIL_SYNC_INTERVAL_MS || 60000)); }).catch(e => { console.error('Database connection failed:', e.message); process.exit(1); });
