...
import adminApiSecretsRoutes from './routes/adminApiSecrets.js';
import adminWebhookSettingsRoutes from './routes/adminWebhookSettings.js';
...
import { startGmailIdleWatchers } from './services/gmailImapIdleWatcher.js';
...
app.use('/api/v1/admin/api-settings', adminApiSecretsRoutes);
app.use('/api/v1/admin/webhook-settings', adminWebhookSettingsRoutes);
...
async function ensureBootstrapAdmin() { const email = String(process.env.ADMIN_EMAIL || '').trim().toLowerCase(), password = String(process.env.ADMIN_PASSWORD || ''); if (!email || !password) { console.log('Admin bootstrap skipped: ADMIN_EMAIL/ADMIN_PASSWORD not configured.'); return; } if (password.length < 8) throw new Error('ADMIN_PASSWORD must be at least 8 characters'); let existing = await User.findOne({ email }).select('+passwordHash'); if (!existing) { const passwordHash = await bcrypt.hash(password, 12); await User.create({ name: process.env.ADMIN_NAME || 'Administrator', email, passwordHash, role: 'admin', status: 'active', trialStartedAt: null, trialEndsAt: null, plan: null, planStatus: 'NONE' }); console.log(`Bootstrap admin created: ${email}`); return; } let changed = false; if (existing.role !== 'admin') { existing.role = 'admin'; changed = true; } if (existing.status !== 'active') { existing.status = 'active'; changed = true; } const passwordMatches = existing.passwordHash ? await bcrypt.compare(password, existing.passwordHash) : false; if (!passwordMatches) { existing.passwordHash = await bcrypt.hash(password, 12); changed = true; } if (existing.trialStartedAt || existing.trialEndsAt || existing.plan || existing.planStatus !== 'NONE' || existing.planStartedAt || existing.planExpiresAt) { existing.trialStartedAt = null; existing.trialEndsAt = null; existing.plan = null; existing.planStartedAt = null; existing.planExpiresAt = null; existing.planStatus = 'NONE'; changed = true; } if (process.env.ADMIN_NAME && existing.name !== process.env.ADMIN_NAME) { existing.name = process.env.ADMIN_NAME; changed = true; } if (changed) await existing.save({ validateBeforeSave: false }); console.log(`Bootstrap admin ready: ${email}`); }
...
async function gmailAutoSyncTick() { const settings = await GatewaySettings.findOne({ key: 'global' }).lean(); const enabled = settings?.gmailAutoSync !== false && process.env.GMAIL_AUTO_SYNC !== 'false'; if (!enabled) return; try { const result = await verifyAllConnectedGmails(); if (result.confirmed) console.log('Gmail IMAP fallback verification:', result); } catch (e) { console.error('Gmail IMAP fallback sync failed:', e.message); } }

async function repairUniqueIndexes() { try { const orderIndexes = await Order.collection.indexes(); const legacyOrder = orderIndexes.find(index => index.name === 'merchant_1_utr_1'); if (legacyOrder && !legacyOrder.partialFilterExpression) { await Order.collection.dropIndex('merchant_1_utr_1'); console.log('Dropped legacy Order merchant_1_utr_1 index.'); } const receiptIndexes = await PaymentReceipt.collection.indexes(); const legacyReceipt = receiptIndexes.find(index => index.name === 'merchant_1_utr_1'); if (legacyReceipt && !legacyReceipt.partialFilterExpression) { await PaymentReceipt.collection.dropIndex('merchant_1_utr_1'); console.log('Dropped legacy PaymentReceipt merchant_1_utr_1 index.'); } await Order.syncIndexes(); await PaymentReceipt.syncIndexes(); console.log('Order and PaymentReceipt indexes synchronized.'); } catch (e) { console.error('Unique index repair failed:', e.message); throw e; } }

connectDatabase().then(async () => { await repairUniqueIndexes(); await ensureBootstrapAdmin(); app.listen(port, () => console.log(`OmniUPI API listening on port ${port}`)); await expireSubscriptions().catch(e => console.error('Initial expiry check failed:', e.message)); startGmailIdleWatchers().then(result => console.log('Gmail IMAP IDLE watchers started:', result)).catch(e => console.error('Gmail IMAP IDLE watcher startup failed:', e.message)); setInterval(() => startGmailIdleWatchers().catch(e => console.error('Gmail watcher refresh failed:', e.message)), 60000); setInterval(() => expireSubscriptions().catch(e => console.error('Subscription expiry check failed:', e.message)), Number(process.env.SUBSCRIPTION_EXPIRY_CHECK_MS || 60000)); setInterval(gmailAutoSyncTick, Number(process.env.GMAIL_SYNC_INTERVAL_MS || 300000)); }).catch(e => { console.error('Database connection failed:', e.message); process.exit(1); });