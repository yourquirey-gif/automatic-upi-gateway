...
function syncRows(merchants){
  const heading=[...document.querySelectorAll('h2')].find(x=>x.textContent.trim()==='Merchants');
  if(!heading)return;
  ...
  const upiIndex=headers.findIndex(x=>x.includes('upi id'));const verificationIndex=headers.findIndex(x=>x.includes('verification'));const statusIndex=headers.findIndex(x=>x==='status'||x.includes('status'));const actionIndex=headers.findIndex(x=>x.includes('action'));
...
const observer=new MutationObserver(()=>setTimeout(addDeleteButtons,120));observer.observe(document.body,{childList:true,subtree:true});setInterval(addDeleteButtons,1500);setTimeout(addDeleteButtons,700);
})();
