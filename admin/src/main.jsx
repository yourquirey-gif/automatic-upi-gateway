...
import { CreditCard, LayoutDashboard, LogIn, Save, ShieldCheck, Trash2, WalletCards } from 'lucide-react';
import { adminApi, adminLogin } from './api';
import './styles.css';

const emptyPlan = { name: '', price: '', durationDays: 30, transactionFeePercent: 0, features: [], featuresText: '', active: true };
...
function Login({ onLogin }) {
  ...
  const submit = async e => { e.preventDefault(); setBusy(true); setError(''); try { const data = await adminLogin(email, password); onLogin(data.token); } catch (err) { setError(err.message || 'Login failed'); } finally { setBusy(false); } };
...
function Console({ onLogout }) {
  ...
  const load = async () => { try { const [p, s] = await Promise.all([adminApi('/plans'), adminApi('/settings')]); setPlans(p.plans || []); setSettings(s.settings || {}); } catch (err) { if (/auth|token|administrator/i.test(err.message || '')) onLogout(); else setMessage(err.message); } };
  useEffect(() => { load(); }, []);

  const savePlan = async e => {
    e.preventDefault();
    const featureText = draft.featuresText ?? (draft.features || []).join('\n');
    ...
    try { if (editing) await adminApi(`/plans/${editing}`, { method: 'PUT', body: JSON.stringify(payload) }); else await adminApi('/plans', { method: 'POST', body: JSON.stringify(payload) }); setMessage('Plan saved'); setEditing(null); setDraft(emptyPlan); await load(); } catch (err) { setMessage(err.message); }
  };
