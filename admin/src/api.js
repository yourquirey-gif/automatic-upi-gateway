...
function resolveAdminPath(path) {
  ...
  if (p === '/gmail/connect' || p === '/gmail/sync' || p === '/gmail/status') return p;
...
export async function adminApi(path, options = {}) {
  ...
  catch { throw new Error(`Admin API unreachable: ${url}`); }
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) throw new Error(data.message || 'Administrator authentication required');
