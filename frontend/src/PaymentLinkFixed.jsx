...
export function PaymentLinkPageFixed(){
 ...
 const generate=async()=>{
  ...
  try{
   const result=await api('/orders/payment-link',{method:'POST',body:JSON.stringify({merchantId:m.id,customerMobile:mobile,amount:Number(n.toFixed(2)),remark:remark.trim()})});
 ...
 return <Page title="Payment Link Management"><section className="cx-card"><div className="hero"><Link2 size={34}/><div><h1>Payment Link Management</h1><p>Create backend-backed links with real Order IDs.</p></div></div>{!m&&<Warn>Connect Merchant → Verify UPI ID before generating a link.</Warn>}{toast&&<div className="toast">{toast}<button onClick={()=>setToast('')}><X size={15}/></button></div>}<div className="stats"><Stat n="₹0.00" t="TODAY'S COLLECTION"/><Stat n={links.length} t="LINKS TODAY"/><Stat n="1" t="ACCOUNT"/><Stat n={links.length?'Active':'No Links'} t="STATUS"/></div><div className="form"><div className="formhead"><h2>＋ Create New Payment Link</h2><span>◷ Backend Order + Gmail verification</span></div><div className="grid2"><Field label="CUSTOMER NAME"><input value={customer} onChange={e=>setCustomer(e.target.value)} placeholder="Enter customer name"/></Field><Field label="MOBILE NUMBER"><input value={mobile} onChange={e=>setMobile(e.target.value.replace(/\D/g,'').slice(0,10))} placeholder="10-digit mobile"/></Field><Field label="AMOUNT (₹)"><input type="number" min="1" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="0.00"/></Field><Field label="REMARK (OPTIONAL)"><input value={remark} onChange={e=>setRemark(e.target.value)} placeholder="e.g. Gift, Deposit"/></Field></div><button className="primary" onClick={generate} disabled={creating}><Zap size={18}/>{creating?'Creating Order…':'Generate Link'}</button><p className="bind-note">Each link is created by the backend with a unique Order ID. The customer checkout verifies payment against that exact Order ID and amount.</p></div><div className="form"><div className="formhead"><h2>☷ My Links</h2><span>Total: {links.length}</span></div>{!links.length?<div className="empty"><Link2 size={46}/><h3>No Links Found</h3><p>Create your first link to get started.</p></div>:links.map(l=><div className="linkrow" key={l.id}><div><b>{l.customer} • ₹{l.amount}</b><small>{l.orderId||l.id} • {new Date(l.createdAt).toLocaleString('en-IN')}</small><code>{l.link}</code></div><div className="actions"><button onClick={()=>copy(l.link)}><Copy size={15}/> Copy</button><button onClick={()=>window.open(l.link,'_blank')}><ExternalLink size={15}/> Open</button></div></div>)}</div></section></Page>
}
