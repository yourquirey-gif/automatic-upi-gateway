...
import {Check,Clock3,Copy,Download,ExternalLink,Link2,Menu,ShieldCheck,Smartphone,X,Zap} from 'lucide-react';
import {api} from './api';
...
function settingsFor(m){const key=merchantId(m)?`${SETTINGS_KEY}:${merchantId(m)}`:SETTINGS_KEY;return {...DEFAULT,...read(key,{})}}
...
function qrImageUrl(upi){return `https://api.qrserver.com/v1/create-qr-code/?size=320x320&margin=8&data=${encodeURIComponent(upi)}`}
...
export function PaymentLinkPageFixed(){
 ...
 const [links,setLinks]=useState(()=>read(LINKS_KEY,[])),[toast,setToast]=useState(''),[creating,setCreating]=useState(false);
 ...
 const generate=async()=>{
  ...
  if(creating)return;
  setCreating(true);setToast('Creating secure payment order…');
  try{
   const result=await api('/orders/payment-link',{method:'POST',body:JSON.stringify({merchantId:merchantId(m),customerMobile:mobile,amount:Number(n.toFixed(2)),remark:remark.trim()})});
   const order=result?.result;
   if(!order?.orderId||!order?.paymentUrl)throw new Error('Backend did not return a valid Order ID/payment URL.');
   const item={id:order.orderId,orderId:order.orderId,customer:customer.trim(),mobile,amount:Number(order.amount).toFixed(2),remark:remark.trim(),link:order.paymentUrl,createdAt:Date.now(),expires:new Date(order.expiresAt).getTime(),status:order.status||'PENDING'};
   setLinks(p=>[item,...p.filter(x=>x?.orderId!==item.orderId)]);
   setCustomer('');setMobile('');setAmount('');setRemark('');setToast(`Payment link created. Order ID: ${order.orderId}`);
  }catch(error){setToast(error?.message||'Unable to create payment link. Please try again.');}
...
function QrBlock({src,amount}){const qr=qrImageUrl(src);return <div className="qrblock"><b>▦ Scan & Pay</b><div><img src={qr} alt="UPI QR Code for payment"/><div><small>Scan to pay exactly ₹{amount.toFixed(2)}</small><a href={qr} target="_blank" rel="noreferrer"><Download size={15}/> Download QR</a></div></div></div>}
