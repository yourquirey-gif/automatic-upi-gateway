...
export function PaymentLinkPageFixed(){
 ...
 const generate=()=>{
  ...
  const createdAt=Date.now(); const expires=createdAt+5*60*1000;
...
export function PublicPaymentPageFixed({route}){
 ...
 const p=useMemo(()=>({customer:q.get('customer')||'Customer',mobile:q.get('mobile')||'',amount:Number(q.get('amount')||0),remark:q.get('remark')||'',upi:q.get('upi')||'',expires:Number(q.get('expires')||0)}),[q]);
 const s=useMemo(()=>parseSettings(q),[q]);
 const [now,setNow]=useState(Date.now()); const [copied,setCopied]=useState(false);
 useEffect(()=>{const id=setInterval(()=>setNow(Date.now()),250);return()=>clearInterval(id)},[]);
 const remaining=Math.max(0,p.expires-now); const expired=!p.expires||!p.upi||!Number.isFinite(p.amount)||p.amount<=0||remaining<=0; const intent=p.upi?upiUrl(p.upi,p.amount,p.remark,s.brandName):'';
 if(expired)return <PublicShell><div className="expired"><Clock3 size={50}/><h2>Payment Link Expired</h2><p>This payment link was valid for 5 minutes and is no longer active.</p></div></PublicShell>;
 return <PublicShell><div className="public-card"><div className="public-top" style={{background:`linear-gradient(135deg,${s.theme},#6d4bc0)`}}><div className="brand">{s.logo?<img src={s.logo} alt=""/>:<span>▣</span>}<b>{s.brandName}</b><i>Expires in <strong>{formatTime(remaining)}</strong></i></div><small>Amount</small><strong>₹{p.amount.toFixed(2)}</strong></div><div className="public-body"><div className="request"><small>PAYMENT REQUEST</small><h2>{p.customer}</h2>{p.remark&&<p>{p.remark}</p>}</div>{s.showQr&&<QrBlock src={qr(intent)} amount={p.amount}/>} {s.showIntent&&<IntentButtons href={intent}/>}<Instructions text={s.instructions}/><div className="receiving"><small>RECEIVING UPI ID</small><b>{p.upi}</b><button onClick={async()=>{try{await navigator.clipboard.writeText(p.upi);setCopied(true);setTimeout(()=>setCopied(false),1500)}catch{}}}>{copied?<Check size={14}/>:<Copy size={14}/>} {copied?'Copied':'Copy'}</button></div><a className="pay" href={intent}><Smartphone size={18}/> Pay ₹{p.amount.toFixed(2)} via UPI App</a><p className="fixed">🔒 Secure UPI payment • Amount is fixed to this request • No Auto-pay</p></div></div></PublicShell>
}
