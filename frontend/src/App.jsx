...
import { ArrowRight, Check, Menu, Play, Rocket, ShieldCheck, LockKeyhole, UserRoundCheck, Sparkles, BarChart3, Zap, Shield, X } from 'lucide-react';
...
function App() {
  ...
  const authenticated = async (action) => {
    await action();
    setAuth(null);
    alert('Authentication successful. Your merchant account is ready.');
  };
  ...
  return <div className="landing">
    ...
    {auth && <AuthModal mode={auth} onClose={() => setAuth(null)} onLogin={(email,password) => authenticated(() => login(email,password))} onSignup={(name,email,password) => authenticated(() => register(name,email,password))} />}
  </div>;
}
