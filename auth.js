...
function authUrl(mode){return `${AUTH_BASE_URL}/api/v1/auth/google?mode=${encodeURIComponent(mode)}`}
function handleGoogleCallback(){
  const hash=new URLSearchParams(location.hash.replace(/^#/,'').replace(/^\?/,'').split('&').map(x=>x).join('&'));
  const token=hash.get('google_token');
  if(!token)return false;
  localStorage.setItem('omniupi_token',token);
  const merchantId=hash.get('merchant_id');
  if(merchantId)sessionStorage.setItem('google_merchant_id',merchantId);
  const verified=hash.get('merchant_verified');
  if(verified!==null)sessionStorage.setItem('google_merchant_verified',verified);
  history.replaceState(null,document.title,location.pathname+location.search);
  return true;
}
function renderAuth(){
  const googleDone=handleGoogleCallback();
  if(googleDone){document.querySelector('.shell')?.removeAttribute('hidden');if(typeof show==='function')show('Dashboard');else if(location.hash!=='#dashboard')location.hash='dashboard';return;}
  document.querySelector('.shell')?.setAttribute('hidden','hidden');
  let el=document.getElementById('authScreen');
  if(el)return;
  document.body.insertAdjacentHTML('afterbegin',`<div id="authScreen"><div class="auth-card"><div class="auth-brand"><div class="auth-logo">U</div><div><b>OmniUPI</b><small>Secure payment infrastructure</small></div></div><div class="auth-tabs"><button class="auth-tab active" id="loginTab" onclick="setAuthMode('login')">Login</button><button class="auth-tab" id="signupTab" onclick="setAuthMode('signup')">Sign up</button></div><div id="authContent"></div></div></div>`);
  setAuthMode('login');
}
...
async function submitEmailAuth(event,mode){
  ...
  const email=document.getElementById('authEmail')?.value.trim();const password=document.getElementById('authPassword')?.value||'';const name=document.getElementById('authName')?.value.trim()||'';const confirm=document.getElementById('authConfirm')?.value||'';
  if(!email||password.length<8)return false;if(mode==='signup'&&password!==confirm){showAuthError('Passwords do not match.');return false}
  const button=document.getElementById('authSubmit');if(button){button.disabled=true;button.textContent=mode==='login'?'Signing in…':'Creating account…'}
  try{const response=await fetch(apiUrl(`/api/v1/auth/${mode==='login'?'login':'register'}`),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(mode==='login'?{email,password}:{name,email,password})});let data={};try{data=await response.json()}catch{}if(!response.ok||!data.status)throw new Error(data.message||`Authentication failed (${response.status})`);localStorage.setItem('omniupi_token',data.token||'');localStorage.setItem('omniupi_user',JSON.stringify(data.user||{}));if(data.trial)localStorage.setItem('omniupi_trial',JSON.stringify(data.trial));continueToPanel();}catch(error){showAuthError(error.message||'Unable to connect to the backend. Please try again.');}finally{if(button){button.disabled=false;button.textContent=mode==='login'?'Login':'Create account'}}
  return false;
}
function continueWithGoogle(mode){if(AUTH_BASE_URL){location.href=authUrl(mode);return}showAuthError('Google OAuth backend is not configured yet.')}
...
function replaceMerchantModal(){
  const modal=document.getElementById('merchantModal');if(!modal)return;const card=modal.querySelector('.modal-card');if(!card)return;
...
function merchantGoogleConnect(){
  ...
  if(AUTH_BASE_URL){location.href=`${AUTH_BASE_URL}/api/v1/auth/google/merchant?upi=${encodeURIComponent(upi)}&mobile=${encodeURIComponent(mobile)}`;return}
